...
images_path = 'drive/MyDrive/FER2013'        #Path of the dataset present in google drive
print('The different emotions to be detected are: ') 
print(os.listdir(images_path))

total = sum(len(files) for _, _, files in os.walk(r'drive/MyDrive/FER2013'))        #Getting the total count of the images present
...
def create_model():
    ...
    model = Sequential()
    model.add(Conv2D(6, (5, 5), input_shape=input_shape, padding='same', activation = 'relu'))        #Convolutional layer with Relu activation function
    model.add(MaxPooling2D(pool_size=(2, 2)))           #Maxpooling

    model.add(Conv2D(16, (5, 5), padding='same', activation = 'relu'))          #Convolutional layer with Relu activation function
    model.add(Activation('relu'))
    model.add(MaxPooling2D(pool_size=(2, 2)))           #Maxpooling
    
    model.add(Conv2D(20, (5,5), padding='same', activation = 'relu'))         #Convolutional layer with Relu activation function
    model.add(MaxPooling2D(pool_size=(2, 2)))          #Maxpooling

    model.add(Conv2D(64, (3, 3) , padding='same', activation = 'relu'))            #Convolutional layer with Relu activation function
    model.add(MaxPooling2D(pool_size=(2, 2)))            #Maxpooling

    model.add(Flatten())            #Flattening the input
    model.add(Dense(128, activation = 'relu'))          #Dense layer
    
    model.add(Dropout(0.5))          #Here, some randomly selected neurons are ignored
    model.add(Dense(5, activation = 'softmax'))
    
    model.compile(loss='categorical_crossentropy', metrics=['accuracy'],optimizer='RMSprop')         #Compile defines the loss function, the optimizer and the metrics
